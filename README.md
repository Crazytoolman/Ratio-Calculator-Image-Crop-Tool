...
- **Draw Things 扩图底板**: 专为 AI 扩图场景设计。快速生成符合比例要求的纯绿背景底板，完美匹配 Draw Things 等工具中通过纯绿背景训练的扩图 LoRA。实现“布局 -> 填充绿色 -> 粘贴生成”的高效工作流。
- **ACE++ 完美搭档**: 新增的“事先拼图”工作流特别适合 **ACE (ACE++)** 等参考生成。支持在单张画布上预先拼接多个人物或场景素材，作为指导性输入图。
- **Qwen Image Edit 助手**: 针对 **Qwen Image Edit** 等多模态编辑模型，利用拼接功能提前布局多个人物或场景关系，实现更精准的局部重绘和画面指导生成。
...
### 🖼️ 扩图底板 (New!)
- **智能底板生成**: 基于内置的比例计算器，一键生成 4:3、16:9 或自定义比例的纯绿（#00FF00）画布。
- **多层级交互**: 支持在底板上通过点击、粘贴或拖拽添加图片，自由布局。
- **精准控制**: 允许对悬浮图片进行自由移动、缩放和旋转，方便精细化图层管理。
- **导出与集成**: 确认合成后可直接下载或复制结果，无缝衔接至 Draw Things 等 AI 创作软件。
