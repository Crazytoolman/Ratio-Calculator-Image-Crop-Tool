...
- **一键交换宽高**: 自定义比例支持快速交换宽高，例如 `768 × 1024` 一键切换为 `1024 × 768`。
...
- **自定义裁切模式**:
  - **固定尺寸**: 自定义输入如 `768 × 1024` 时，裁切信息与导出 PNG 严格保持该目标尺寸。
  - **比例裁切**: 自定义输入作为比例使用，可继续拖动角点进行等比缩放裁切。
  - **宽高互换**: 在自定义输入中一键交换宽高，并同步刷新预览、裁切框与导出目标。
- **圆角调节**:
...
- **自定义比例互换**: 扩图底板的自定义宽高同样支持一键交换，例如快速从竖版底板切换为横版底板。
